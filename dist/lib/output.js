...
export function mismatchingVersionsToOutput(mismatchingDependencyVersions) {
    ...
    const tables = mismatchingDependencyVersions
        ...
        const hasMultipleUsageCounts = !usageCounts.every((count) => count === highestUsageCount);
        const rows = object.versions
            ...
            const packageNames = versionObject.packages.map((package_) => package_ === '.' ? '(Root)' : package_);
        ...
        .join('');
    return [
        `Found ${mismatchingDependencyVersions.length} ${mismatchingDependencyVersions.length === 1 ? 'dependency' : 'dependencies'} with mismatching versions across the workspace. Fix with \`--fix\`.`,
        tables,
    ].join('\n');
}
